...
function createCalendar(layout, firstDay, numbDays, monthNum, yearNum) { 
	...
	calendarString += '<tr><td class=\"calendar-btn\" colspan="2"><span onClick=\"changedate(\'prevyr\', \'full\')\">« <span class="btn-change-date">' + prev_year + '<\/span><\/span><\/td><td colspan="3"></td><td class=\"calendar-btn\" colspan="2"><span onClick=\"changedate(\'nextyr\', \'full\')\"><span class="btn-change-date">' + next_year + '<\/span> »<\/span><\/td></tr>';
	calendarString += '<tr>';
...
function showEventDetail(id, layout, day, month, year) {
	...
	if (layout == 'full') {
		...
		if (tiva_events[id].description) {
			var event_desc = '<div class="event-desc"><b>Corporate Action : </b>' + tiva_events[id].description + '</div>';
		} else {
			var event_desc = '';
		}
		// Start date
		...
		if (date_start == 'sunday') {
			var event_day = '<b>Date : </b>' + wordDay[day.getDay()];
		} else {
			
			if (day.getDay() > 0) {
				var event_day = '<b>Date : </b>' + wordDay[day.getDay() - 1];
			} else {
				var event_day = '<b>Date : </b>' + wordDay[6];
			}
		...
		// Event time
		if (tiva_events[id].time) {
			var event_time = '<i class="fa fa-clock-o"></i><b>Time : </b>' + tiva_events[id].time;
		} else {
			var event_time = '';
		}
		...
		if (tiva_events[id].location) {
			var event_location = '<i class="fa fa-map-marker"></i><b>Venue :</b> ' + tiva_events[id].location;
		} else {
			var event_location = '';
		}
		
		// Event Issuer description
		
		if (tiva_events[id].issue_description) {
			var event_issue_description = '<div class="event-desc"><b>Issue Description :</b> ' + tiva_events[id].issue_description + '</div>';
		} else {
			var event_issue_description = '';
		}
		
		// Event Issuer
		if (tiva_events[id].issuer) {
			var event_issuer = '<div class="event-desc"><b>Issuer : </b>' + tiva_events[id].issuer + '</div>';
		} else {
			var event_issuer = '';
		}	
		// Event Series
		if (tiva_events[id].series) {
			var event_series = '<div class="event-desc"><b>Series : </b>' + tiva_events[id].series + '</div>';
		} else {
			var event_series = '';
		}
		// Event Amount
		if (tiva_events[id].amount) {
			var event_amount = '<div class="event-desc"><b>Issue Amount : </b>' + tiva_events[id].amount + '</div>';
		} else {
			var event_amount = '';
		}	
		// Event Issue Date
		if (tiva_events[id].issue_date) {
			var event_issue_date = '<div class="event-desc"><b>Corporate Action Date : </b>' + tiva_events[id].issue_date + '</div>';
		} else {
			var event_issue_date = '';
		}
		// Event Coupon
		if (tiva_events[id].coupon) {
			var event_coupon = '<div class="event-desc"><b>Coupon : </b>' + tiva_events[id].coupon + '</div>';
		} else {
			var event_coupon = '';
		}
		// Event URL
		if (tiva_events[id].url) {
			var event_url = '<div class="event-desc"><b> Document Link : </b>' + tiva_events[id].url + '</div>';
		} else {
			var event_url = '';
		}
		jQuery('.tiva-event-detail-full').html(	'<div class="event-item">'
													...
													+ '<div class="event-image">' + event_image + '</div>'
											   		+ '<div class="event-name">' + tiva_events[id].name + '</div>'
													+ '<div class="event-issuer">' + event_issuer + '</div>' + '<div class="event_issue_description">' + event_issue_description + '</div>'+ event_desc
													+ '<div class="event_series">' + event_series + '</div>'
											   		+ '<div class="event-amount">' + event_amount + '</div>'
											        + '<div class="event-issue_date">' + event_issue_date + '</div>'
											   		+ '<div class="event-coupon">' + event_coupon + '</div>'
											   		+ '<div class="event-location">' + event_location + '</div>'
													+ '<div class="event-date">' + event_day + ', ' + event_date + event_end_time  + '</div>'
													+ '<div class="event-time">' + event_time + '</div>'
																								  
											   + '<div class="event-url" target="_blank">' + event_url + '</div>'
											);
	} else {
		...
		for (var i = 0; i < events.length; i++) {
			if (typeof events[i] != "undefined") {
				...
				if (tiva_events[events[i].id].isssue_description) {
					var event_issue_decription = '<div class="event-desc">' + tiva_events[events[i].id].issue_description + '</div>';
				} else {
					var event_issue_decription = '';
				}
				// Event Issuer
				if (tiva_events[events[i].id].issuer) {
					var event_issuer = '<div class="event-issuer">' + tiva_events[events[i].id].issuer + '</div>';
				} else {
					var event_issuer = '';
				}		
				// Event Series
				if (tiva_events[events[i].id].series) {
					var event_series = '<div class="event-series">' + tiva_events[events[i].id].series + '</div>';
				} else {
					var event_series = '';
				}	
				// Event Amount
				if (tiva_events[events[i].id].amount) {
					var event_amount = '<div class="event-amount">' + tiva_events[events[i].id].amount + '</div>';
				} else {
					var event_amount = '';
				}
				// Event Issues Date
				if (tiva_events[events[i].id].issue_date) {
					var event_issue_date = '<div class="event-issue_date">' + tiva_events[events[i].id].issue_date + '</div>';
				} else {
					var event_issue_date = '';
				}	
				// Event Coupon
				if (tiva_events[events[i].id].coupon) {
					var event_coupon = '<div class="event-coupon">' + tiva_events[events[i].id].coupon + '</div>';
				} else {
					var event_coupon = '';
				}	
				// Event URL
				if (tiva_events[events[i].id].url) {
					var event_url = '<div class="event-url">' + tiva_events[events[i].id].url + '</div>';
				} else {
					var event_url = '';
				}				
				jQuery('.tiva-event-detail-compact').append( 	'<div class="event-item">'
																	+ '<div class="event-image">' + event_image + '</div>'
																	+ '<div class="event-name">' + tiva_events[events[i].id].name + '</div>'
																	+ '<div class="event-issuer">' + event_issuer + '</div>' + event_desc
																	 + '<div class="event-series">' + event_series + '<br></div>'
											   						 + '<div class="event-amount">' + event_amount + '<br></div>'
											   						  + '<div class="event-issue_date">' + event_issue_date + '<br></div>'
											   						  + '<div class="event-coupon">' + event_coupon + '<br></div>'
											   						  + '<div class="event-location">' + event_location + '</div>'
																	+ '<div class="event-date"><i class="fa fa-calendar-o"></i>' + event_day + ', ' + event_date + event_end_time + '</div>'
																	+ '<div class="event-time">' + event_time + '</div>'
